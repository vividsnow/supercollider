...
//				HIDGetUsageName (IOHIDElementGetUsagePage(devElement), IOHIDElementGetUsage(devElement), cstrElementName);
                ...
                CFStringRef tmpRef;
                tmpRef = HIDCopyUsageName(IOHIDDevice_GetPrimaryUsagePage(pCurrentHIDDevice), IOHIDDevice_GetPrimaryUsage(pCurrentHIDDevice));
                if(tmpRef){
                    CFStringGetCString(tmpRef, cstrElementName, 256, kCFStringEncodingASCII);
                    CFRelease(tmpRef);                    
                }
...
int prHIDBuildDeviceList(VMGlobals *g, int numArgsPushed)
{
    ...
    // huh - this doesn't make sense to me - connect a device and test
	if(!result) {
...
//        HIDGetUsageName (IOHIDDevice_GetPrimaryUsagePage(pCurrentHIDDevice), IOHIDDevice_GetPrimaryUsage(pCurrentHIDDevice), tmp);
        ...
        *tmp = 0;
        ...
        if(tmpRef){
            CFStringGetCString(tmpRef, tmp, 256, kCFStringEncodingASCII);
            CFRelease(tmpRef);
        }
		devstring = newPyrString(g->gc, tmp, 0, true);
		SetObject(devNameArray->slots+devNameArray->size++, devstring);
